...
| Sheetal does not offer standalone private sessions. | Founder correction, 2026-09-01 | FIX BEFORE LAUNCH | P1 | `/offerings`, `/begin`, offer charts, future commerce schema | Public private-work language is now container-based: 6, 9, or 12 sessions; 3 sessions only by exception. No single-session, drop-in, or one-off private offer is presented. | 12F suite checks private container copy and rejects single-session language on `/offerings`. | Codex | Pricing and checkout remain blocked until an approved commerce sprint. | Current branch |
...
- Private work with Sheetal begins with a container and continuity over time. Public packages are 6, 9, or 12 sessions; 3 sessions is exception-only.
