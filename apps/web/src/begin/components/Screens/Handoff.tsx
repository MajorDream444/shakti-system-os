...
export default function Handoff({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    PersistenceService.write(STORAGE_KEYS.pathEmail, email);
    PersistenceService.write(STORAGE_KEYS.pathWhatsapp, whatsapp);
    PersistenceService.write(STORAGE_KEYS.pathPathway, pathway);
    PersistenceService.write(STORAGE_KEYS.pathLongings, longings.join(', '));
    PersistenceService.write(STORAGE_KEYS.pathReflection, reflection);

    if (env.sheetEndpoint) {
      try {
        await fetch(env.sheetEndpoint, {
          method: 'POST',
          mode: 'no-cors',
        ...
        // Submission still shows success because PersistenceService keeps a local backup.
      }
    }
